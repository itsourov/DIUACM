...
export const processSingleAtcoderContest = task({
  ...
  run: async ({ 
    ...
    try {
      ...
      const users: UserWithAtcoderHandle[] = usersWithHandles
        .filter((user): user is UserWithAtcoderHandle & { atcoderHandle: string } => 
          user.atcoderHandle !== null
        );

      if (users.length === 0) {
        logger.warn(`No users found with AtCoder handles for event: ${eventInfo.title}`);
      ...
      const stats = await processAtcoderUserStats(contestInfo, contestId, users, eventInfo.id);
...
export const updateAtcoderContestStats = schedules.task({
  ...
  description: "Updates solve statistics for AtCoder contests by calculating contest solves and upsolves for each user",
  // Run every 6 hours
  cron: "0 */6 * * *", 
  ...
  run: async (payload) => {
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error("❌ Error in AtCoder Contest Update task", { error: errorMessage });
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    ...
    const batchResults = await Promise.all(batch.map(async (user) => {
      try {
        ...
        const submissionsUrl = `${ATCODER_API.SUBMISSIONS}?user=${encodeURIComponent(user.atcoderHandle)}&from_second=${contestInfo.start_epoch_second}`;
        ...
        const contestSubmissions = submissions.filter(sub => sub.contest_id === contestId);
