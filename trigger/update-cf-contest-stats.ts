...
export const processSingleCodeforcesContest = task({
  ...
  run: async ({ 
    ...
    try {
      ...
      const users: UserWithCodeforcesHandle[] = usersWithHandles
        .filter((user): user is UserWithCodeforcesHandle & { codeforcesHandle: string } => 
          user.codeforcesHandle !== null
        );

      if (users.length === 0) {
        logger.warn(`No users found with CF handles for event: ${eventInfo.title}`);
      ...
      const stats = processContestResults(eventInfo.id, users, contestResult.data);
...
export const updateCodeforcesContestStats = schedules.task({
  ...
  description: "Updates solve statistics for Codeforces contests by calculating contest solves and upsolves for each user",
  // Run every 6 hours
  cron: "0 */6 * * *", 
  ...
  run: async (payload) => {
    try {
      ...
      logger.info("🚀 Starting Codeforces Contest Statistics Update", { 
        ...
        lastRun: payload.lastTimestamp ? new Date(payload.lastTimestamp).toISOString() : "First run" 
      });
      ...
      for (let i = 0; i < events.length; i++) {
        ...
        const output = result.ok ? result.output : { status: "failed", reason: "task-failed" };
        results.push({
          eventId: event.id,
          title: event.title,
          ...output
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error("❌ Error in Codeforces Contest Update task", { error: errorMessage });
...
async function fetchCodeforcesStandings(
...
): Promise<{ success: true; data: CodeforcesApiResponse } | { success: false; error: string }> {
  ...
  try {
    const url = `https://codeforces.com/api/contest.standings?contestId=${contestId}&showUnofficial=true&handles=${encodeURIComponent(handles)}`;
...
function processContestResults(
  ...
  for (const user of users) {
    ...
    const contestRow = rows.find(row => 
      row.party.members[0]?.handle.toLowerCase() === user.codeforcesHandle.toLowerCase() &&
 ...
 */
async function updateSolveStatsInDatabase(
  stats: { solveStats: SolveStatData[] }
): Promise<void> {
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    await Promise.all(chunk.map(stat => 
      prisma.userSolveStatOnEvent.upsert({
        where: {
          userId_eventId: {
            userId: stat.userId,
