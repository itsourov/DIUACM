...
async function seedUsers() {
    ...
    for (let i = 0; i < 150; i++) {
        ...
        userList.push({
            ...
            startingSemester: getRandomItem(['Spring 2020', 'Fall 2020', 'Spring 2021', 'Fall 2021', 'Spring 2022', 'Fall 2022', 'Spring 2023', 'Fall 2023', 'Spring 2024', 'Fall 2024']),
            department: getRandomItem(['Computer Science & Engineering', 'Electrical & Electronic Engineering', 'Software Engineering', 'Information Technology']),
            studentId: `${getRandomItem(['CSE', 'EEE', 'SWE', 'IT'])}-${faker.number.int({ min: 2020, max: 2024 })}-${faker.number.int({ min: 100, max: 999 })}`,
            maxCfRating: Math.random() > 0.5 ? faker.number.int({ min: 800, max: 2800 }) : null,
        });
    }
...
async function seedRolesAndPermissions() {
    ...
    const permissionList = [
        ...
        { name: 'manage_contests', description: 'Can create, edit, and delete contests' },
        { name: 'manage_events', description: 'Can create, edit, and delete events' },
        { name: 'manage_teams', description: 'Can create, edit, and delete teams' },
        { name: 'manage_galleries', description: 'Can create, edit, and delete galleries' },
        { name: 'manage_blog_posts', description: 'Can create, edit, and delete blog posts' },
        { name: 'manage_trackers', description: 'Can create, edit, and delete trackers' },
        { name: 'view_admin_panel', description: 'Can access admin panel' },
        { name: 'moderate_content', description: 'Can moderate user-generated content' },
    ...
    const moderatorPermissions = createdPermissions.filter(p =>
        ['moderate_content', 'view_admin_panel', 'manage_blog_posts'].includes(p.name)
    );
    ...
    const organizerPermissions = createdPermissions.filter(p =>
        ['manage_contests', 'manage_events', 'manage_teams', 'view_admin_panel'].includes(p.name)
    );
...
async function seedContests(galleryList: any[]) {
    ...
    const contestList = [];
    for (let i = 0; i < 40; i++) {
        const contestName = `${faker.company.name()} Programming Contest ${faker.number.int({ min: 2020, max: 2024 })}`;
        contestList.push({
            name: contestName,
...
async function seedTeams(contestList: any[], userList: any[]) {
    ...
    for (const contest of createdContests) {
        ...
        for (let i = 0; i < teamCount; i++) {
            ...
            const team = {
                ...
                rank: Math.random() > 0.5 ? faker.number.int({ min: 1, max: teamCount }) : null,
                solveCount: Math.random() > 0.5 ? faker.number.int({ min: 0, max: 12 }) : null,
            };
            teamList.push(team);
        }
    }
    ...
    console.log(`✅ Created ${teamList.length} teams with ${teamUserList.length} team memberships`);
...
async function seedEvents() {
    ...
    const eventList = [];
    for (let i = 0; i < 50; i++) {
        const startingAt = faker.date.between({ from: '2020-01-01', to: '2024-12-31' });
        const endingAt = new Date(startingAt);
        endingAt.setHours(endingAt.getHours() + faker.number.int({ min: 2, max: 8 }));
    ...
    for (const tracker of createdTrackers) {
        ...
        for (let i = 0; i < rankListCount; i++) {
            ...
            rankListList.push({
                ...
                weightOfUpsolve: faker.number.float({ min: 0.1, max: 1.0, fractionDigits: 2 }),
                order: i + 1,
...
async function seedJunctionTables(eventList: any[], rankListList: any[], userList: any[]) {
    ...
    for (const event of createdEvents) {
        const selectedRankLists = getRandomItems(createdRankLists, faker.number.int({ min: 1, max: 3 }));
    ...
    for (const rankList of createdRankLists) {
        const selectedUsers = getRandomItems(createdUsers, faker.number.int({ min: 10, max: 50 }));
        for (const user of selectedUsers) {
    ...
    for (const event of createdEvents) {
        const selectedUsers = getRandomItems(createdUsers, faker.number.int({ min: 5, max: 30 }));
        for (const user of selectedUsers) {
    ...
    for (const event of createdEvents) {
        const selectedUsers = getRandomItems(createdUsers, faker.number.int({ min: 5, max: 25 }));
        for (const user of selectedUsers) {
    ...
    console.log(`✅ Created junction table entries: ${eventRankListList.length} event-ranklist, ${rankListUserList.length} ranklist-user, ${eventUserAttendanceList.length} event-attendance, ${userSolveStatList.length} user-solve-stats`);
}
