...
function handleDbError(error: unknown): ActionResult {
    ...
    if (error instanceof Error) {
        if (error.message.includes("Duplicate entry")) {
            return { success: false, error: "A ranklist with this keyword already exists in this tracker" };
        }
...
async function validatePermission(): Promise<ActionResult | null> {
    if (!(await hasPermission("TRACKERS:MANAGE"))) {
        return { success: false, error: "You don't have permission to manage trackers" };
    }
    return null;
}

export async function createRanklist(trackerId: number, values: RanklistFormValues): Promise<ActionResult> {
    try {
        ...
        const result = await db.insert(rankLists).values({
            trackerId,
        ...
        return {
            success: true,
            data: { ...validatedFields, id: result[0].insertId, trackerId },
...
export async function updateRanklist(id: number, trackerId: number, values: RanklistFormValues): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
...
export async function getRanklist(id: number, trackerId: number): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
...
export async function attachEventToRanklist(ranklistId: number, eventId: number, weight: number): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
        ...
        const existing = await db
            ...
            .from(eventRankList)
            .where(and(eq(eventRankList.rankListId, ranklistId), eq(eventRankList.eventId, eventId)))
            .limit(1);

        if (existing.length > 0) {
            return { success: false, error: "Event is already attached to this ranklist" };
        }
...
export async function detachEventFromRanklist(ranklistId: number, eventId: number): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
...
export async function updateEventWeight(ranklistId: number, eventId: number, weight: number): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
...
export async function attachUserToRanklist(ranklistId: number, userId: string, score: number = 0): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
        ...
        const existing = await db
            ...
            .from(rankListUser)
            .where(and(eq(rankListUser.rankListId, ranklistId), eq(rankListUser.userId, userId)))
            .limit(1);

        if (existing.length > 0) {
            return { success: false, error: "User is already attached to this ranklist" };
        }
...
export async function detachUserFromRanklist(ranklistId: number, userId: string): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
...
export async function updateUserScore(ranklistId: number, userId: string, score: number): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
...
export async function getAttachedEvents(ranklistId: number): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
...
export async function getAvailableEvents(ranklistId: number): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
...
export async function getAttachedUsers(ranklistId: number): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
...
export async function getAvailableUsers(ranklistId: number, search?: string): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
