...
export async function generateMetadata({ params }: UsersPageProps): Promise<Metadata> {
    ...
    return {
        title: `${(ranklist as { keyword?: string })?.keyword || 'Ranklist'} Users - ${(tracker as { title?: string })?.title || 'Tracker'} | DIU ACM Admin`,
        description: `Manage users for ranklist ${(ranklist as { keyword?: string })?.keyword || 'ranklist'}`,
    };
}

async function DetachUserButton({ ranklistId, userId, userName }: { ranklistId: number; userId: string; userName: string }) {
        ...
        <AlertDialog>
            ...
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        Are you sure you want to detach &quot;{userName}&quot; from this ranklist?
                        This will remove their score and all data. This action cannot be undone.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
...
async function EditScoreButton({ ranklistId, userId, userName, currentScore }: { ranklistId: number; userId: string; userName: string; currentScore: number }) {
                ...
                <CardHeader>
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <CardDescription>
                                {users.length > 0
                                    ? `${users.length} user${users.length !== 1 ? 's' : ''} attached to this ranklist`
                                    : "No users attached"}
                            </CardDescription>
                        </div>
                    </div>
                    ...
                    {users.length === 0 ? (
                        <div className="text-center py-12">
                            ...
                            <p className="text-sm text-muted-foreground mb-6 max-w-sm mx-auto">
                                Start by attaching users to this ranklist to track their progress and scores.
                            </p>
                            <AttachUserDialog ranklistId={ranklistId} />
                        </div>
                    ) : (
