...
import { getRanklist, getAttachedEvents, detachEventFromRanklist, updateEventWeight } from "../../actions";
...
export async function generateMetadata({ params }: EventsPageProps): Promise<Metadata> {
    ...
    return {
        title: `${(ranklist as { keyword?: string })?.keyword || 'Ranklist'} Events - ${(tracker as { title?: string })?.title || 'Tracker'} | DIU ACM Admin`,
        description: `Manage events for ranklist ${(ranklist as { keyword?: string })?.keyword || 'ranklist'}`,
    };
}

async function EditWeightButton({ ranklistId, eventId, eventTitle, currentWeight }: { ranklistId: number; eventId: number; eventTitle: string; currentWeight: number }) {
                    ...
                    <div className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground">
                                Weight between 0.0 and 1.0 determines the importance of this event
                            </p>
                        </div>
                    </div>
...
async function DetachEventButton({ ranklistId, eventId, eventTitle }: { ranklistId: number; eventId: number; eventTitle: string }) {
        ...
        <AlertDialog>
            ...
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        Are you sure you want to detach &quot;{eventTitle}&quot; from this ranklist?
            ...
            <div className="space-y-2">
                <Breadcrumb>
                    <BreadcrumbList>
                        ...
                        <BreadcrumbItem>
                            <BreadcrumbLink asChild>
                                <Link href={`/admin/trackers/${trackerId}/ranklists/${ranklistId}/edit`}>
                    ...
                    {events.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-10 text-muted-foreground">
                            ...
                            <p className="mb-4 mt-2 text-center text-sm text-muted-foreground max-w-xs">
                                Start by attaching events to this ranklist to track user participation and scores.
                            </p>
                            <AttachEventDialog ranklistId={ranklistId} />
                        </div>
                    ) : (
                                            ...
                                            <TableRow key={item.eventId}>
                                                        ...
                                                        {item.event.description && (
                                                            <p className="text-xs text-muted-foreground truncate max-w-xs">
                                                                {item.event.description.length > 60
                                                                    ? `${item.event.description.substring(0, 60)}...`
                                                                    : item.event.description}
                                                            </p>
                                                        )}
                                                    </div>
                                                ...
                                                <TableCell>
                                                    <span className="text-sm">
                                                        {format(new Date(item.event.startingAt), "MMM dd, yyyy")}
                                                    </span>
                                                </TableCell>
                                                <TableCell>
