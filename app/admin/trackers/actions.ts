...
function handleDbError(error: unknown): ActionResult {
    ...
    if (error instanceof Error) {
        if (error.message.includes("Duplicate entry")) {
            return { success: false, error: "A tracker with this title or slug already exists" };
        }
    }
...
export async function createTracker(values: TrackerFormValues): Promise<ActionResult> {
    try {
        ...
        if (existingByTitle.length > 0) {
            return { success: false, error: "A tracker with this title already exists" };
        }
        ...
        if (existingBySlug.length > 0) {
            return { success: false, error: "A tracker with this slug already exists" };
        }
        ...
        return {
            success: true,
            data: { ...validatedFields, id: result[0].insertId },
...
export async function updateTracker(id: number, values: TrackerFormValues): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
