...
function handleDbError(error: unknown): ActionResult {
    ...
    if (error instanceof Error) {
        ...
        if (error.message.includes("Duplicate entry")) {
            return { success: false, error: "A blog with this title or slug already exists" };
        }
    }
...
async function validatePermission(): Promise<ActionResult | null> {
    if (!(await hasPermission("manage_blog_posts"))) {
        return { success: false, error: "You don't have permission to manage blog posts" };
    }
    return null;
}
...
export async function createBlog(values: BlogFormValues): Promise<ActionResult> {
    try {
        ...
        const dbValues = {
            ...
            publishedAt: validatedFields.publishedAt && validatedFields.publishedAt !== ""
...
export async function updateBlog(id: number, values: BlogFormValues): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
...
export async function getBlog(id: number): Promise<ActionResult> {
    try {
        const permissionError = await validatePermission();
        if (permissionError) return permissionError;
