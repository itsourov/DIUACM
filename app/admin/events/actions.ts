...
import { events, eventUserAttendance, eventRankList } from "@/db/schema";
import { revalidatePath } from "next/cache";
import { eq, or, like, count, desc } from "drizzle-orm";
import { eventFormSchema, type EventFormValues } from "./schemas/event";
import type { EventType, ParticipationScope, VisibilityStatus } from "@/db/schema";
...
function handleDbError<T = unknown>(error: unknown): ActionResult<T> {
    ...
    if (error instanceof Error) {
        ...
        if (error.message.includes("Duplicate entry")) {
            return { success: false, error: "An event with this title already exists" };
        }
...
// Create a new event
export async function createEvent(values: EventFormValues): Promise<ActionResult<EventData>> {
    try {
        ...
        return {
            ...
            data: { id: result.insertId as number, ...data, startingAt } as EventData
        };
    } catch (error) {
        return handleDbError<EventData>(error);
    }
}

// Update an existing event
export async function updateEvent(id: number, values: EventFormValues): Promise<ActionResult<EventData>> {
    try {
        // Check if the user has permission to manage events
        if (!(await hasPermission("EVENTS:MANAGE"))) {
            return { success: false, error: "Unauthorized" };
        ...
        return {
            ...
            data: { id, ...data, startingAt } as EventData
        };
    } catch (error) {
        return handleDbError<EventData>(error);
    }
...
export async function getEvent(id: number): Promise<ActionResult<EventData>> {
    try {
        // Check if the user has permission to manage events
        if (!(await hasPermission("EVENTS:MANAGE"))) {
            return { success: false, error: "Unauthorized" };
...
export async function getEventAttendeesCount(eventId: number): Promise<ActionResult<number>> {
    try {
        if (!(await hasPermission("EVENTS:MANAGE"))) {
            return { success: false, error: "Unauthorized" };
        }
...
export async function getEventRanklistsCount(eventId: number): Promise<ActionResult<number>> {
    try {
        if (!(await hasPermission("EVENTS:MANAGE"))) {
            return { success: false, error: "Unauthorized" };
        }
