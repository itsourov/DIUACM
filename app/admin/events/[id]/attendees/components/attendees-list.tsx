...
type AttendanceWithUser = EventAttendance & {
  user: Pick<
    User,
...
export function AttendeesList({
  ...
  const handleRemoveAttendee = async (attendeeId: string) => {
    try {
      ...
      const response = await removeEventAttendee(attendeeId, eventId);
      if (response.success) {
        setAttendees(attendees.filter((item) => item.id !== attendeeId));
